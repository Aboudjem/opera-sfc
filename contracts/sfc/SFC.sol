...
contract SFC is Initializable, Ownable, StakersConstants, Version {
    ...
    */
    ...
    function getEpochSnapshotValidatorIDs(uint256 epoch)
        public
        view
    ...
    function getEpochSnapshotReceivedStake(uint256 epoch, uint256 validatorID)
        public
        view
        returns (uint256)
    {
    ...
    function getEpochSnapshotAccumulatedRewardPerToken(
        uint256 epoch,
        uint256 validatorID
    ...
    function getEpochSnapshotAccumulatedUptime(
        uint256 epoch,
        uint256 validatorID
    ) public view returns (uint256) {
    ...
    function getEpochSnapshotAccumulatedOriginatedTxsFee(
        uint256 epoch,
        uint256 validatorID
    ) public view returns (uint256) {
    ...
    function getEpochSnapshotOfflineTime(uint256 epoch, uint256 validatorID)
        public
        view
        returns (uint256)
    {
    ...
    function getEpochSnapshotOfflineBlocks(uint256 epoch, uint256 validatorID)
        public
        view
        returns (uint256)
    {
    ...
    */
    ...
    function setGenesisDelegation(
        ...
        if (lockedStake != 0) {
            ...
            LockedDelegation storage ld =
                getLockupInfo[delegator][toValidatorID];
    ...
    */
    ...
    function _isSelfStake(address delegator, uint256 toValidatorID)
        internal
        view
        returns (bool)
    {
    ...
    function _getSelfStake(uint256 validatorID)
        internal
        view
        returns (uint256)
    {
        ...
        returns (bool)
    {
        return
            ...
            (_getSelfStake(validatorID) * (maxDelegatedRatio())) /
                (Decimal.unit());
    }
    ...
    function _rawDelegate(
        ...
        getStake[delegator][toValidatorID] =
            getStake[delegator][toValidatorID] +
            (amount);
    ...
    function undelegate(
        ...
        getValidator[toValidatorID].receivedStake =
            getValidator[toValidatorID].receivedStake -
            (amount);
        totalStake = totalStake - (amount);
        if (getValidator[toValidatorID].status == OK_STATUS) {
    ...
    function getSlashingPenalty(
        ...
        penalty =
            (amount * (Decimal.unit() - refundRatio)) /
            (Decimal.unit()) +
            (1);
        if (penalty > amount) {
            return amount;
    ...
    function withdraw(uint256 toValidatorID, uint256 wrID) external {
        ...
        require(
            _now() >= requestTime + unstakePeriodTime(),
            "not enough time passed"
        );
        require(
            currentEpoch() >= requestEpoch + unstakePeriodEpochs(),
            "not enough epochs passed"
        );

        uint256 amount =
            getWithdrawalRequest[delegator][toValidatorID][wrID].amount;
    ...
    function _calcRawValidatorEpochTxReward(
        ...
        uint256 txReward =
            (epochFee * (txRewardWeight)) / (totalTxRewardWeight);
        // fee reward except contractCommission
        return
            (txReward * (Decimal.unit() - contractCommission())) /
            (Decimal.unit());
    }
    ...
    mapping(address => mapping(uint256 => uint256))
        public claimedRewardUntilEpoch;
    ...
    {
        if (lockupDuration != 0) {
            uint256 maxLockupExtraRatio =
                Decimal.unit() - unlockedRewardRatio();
            uint256 lockupExtraRatio =
                (maxLockupExtraRatio * (lockupDuration)) /
                    (maxLockupDuration());
            reward =
                (fullReward * (unlockedRewardRatio() + lockupExtraRatio)) /
                (Decimal.unit());
            penalty =
                (fullReward * (unlockedRewardRatio() / 2 + lockupExtraRatio)) /
                (Decimal.unit());
        } else {
    ...
    function _nonStashedRewards(address delegator, uint256 toValidatorID)
        internal
        view
        returns (uint256 reward, uint256 penalty)
    {
        uint256 claimedUntil =
            claimedRewardUntilEpoch[delegator][toValidatorID];
        ...
        fullReward = _nonStashedRewardsOf(
            ld.lockedStake,
            toValidatorID,
            claimedUntil,
            lockedUntil
        );
        (uint256 plReward, uint256 plPenalty) =
            _scaleLockupReward(fullReward, ld.duration);
        ...
        (uint256 puReward, uint256 puPenalty) =
            _scaleLockupReward(fullReward, 0);
        ...
        (uint256 wuReward, uint256 wuPenalty) =
            _scaleLockupReward(fullReward, 0);

        return (
            plReward + (puReward) + (wuReward),
            plPenalty + (puPenalty) + (wuPenalty)
        );
    }

    function _nonStashedRewardsOf(
        uint256 stakeAmount,
        uint256 toValidatorID,
        uint256 fromEpoch,
        uint256 toEpoch
        ...
        uint256 claimedRate =
            getEpochSnapshot[fromEpoch].accumulatedRewardPerToken[
                toValidatorID
            ];
        uint256 currentRate =
            getEpochSnapshot[toEpoch].accumulatedRewardPerToken[toValidatorID];
    ...
    {
        (uint256 nonStashedReward, uint256 nonStashedPenalty) =
            _nonStashedRewards(delegator, toValidatorID);
        claimedRewardUntilEpoch[delegator][
            toValidatorID
        ] = _highestPayableEpoch(toValidatorID);
        rewardsStash[delegator][toValidatorID] =
            rewardsStash[delegator][toValidatorID] +
            (nonStashedReward);
        LockedDelegation storage ld = getLockupInfo[delegator][toValidatorID];
    ...
    function _sealEpoch_rewards(
        ...
        _SealEpochRewardsCtx memory ctx =
            _SealEpochRewardsCtx(
        ...
        EpochSnapshot storage prevSnapshot =
            getEpochSnapshot[currentEpoch() - (1)];
        ...
        for (uint256 i = 0; i < validatorIDs.length; i++) {
            ...
            LockedDelegation storage ld =
                getLockupInfo[validatorAddr][validatorID];
            uint256 commissionRewardFull =
                _calcValidatorCommission(rawReward, validatorCommission());
            {
                uint256 lCommissionRewardFull =
                    (commissionRewardFull *
                        getLockedStake(validatorAddr, validatorID)) /
                        getStake[validatorAddr][validatorID];
                uint256 uCommissionRewardFull =
                    commissionRewardFull - lCommissionRewardFull;
                (uint256 lCommissionReward, uint256 lCommissionPenalty) =
                    _scaleLockupReward(lCommissionRewardFull, ld.duration);
                (uint256 uCommissionReward, uint256 uCommissionPenalty) =
                    _scaleLockupReward(uCommissionRewardFull, 0);
                rewardsStash[validatorAddr][validatorID] =
                    rewardsStash[validatorAddr][validatorID] +
                    (lCommissionReward) +
                    (uCommissionReward);
                ld.earlyUnlockPenalty =
                    ld.earlyUnlockPenalty +
                    (lCommissionPenalty) +
                    (uCommissionPenalty);
            }
            ...
            uint256 rewardPerToken =
                (delegatorsReward * (Decimal.unit())) / (totalStake);
            snapshot.accumulatedRewardPerToken[validatorID] =
                prevSnapshot.accumulatedRewardPerToken[validatorID] +
                (rewardPerToken);
            //
            snapshot.accumulatedOriginatedTxsFee[validatorID] =
                prevSnapshot.accumulatedOriginatedTxsFee[validatorID] +
                (originatedTxsFee[i]);
        ...
        onlyDriver
    {
        ...
        for (uint256 i = 0; i < nextValidatorIDs.length; i++) {
            uint256 receivedStake =
                getValidator[nextValidatorIDs[i]].receivedStake;
    ...
    function _calcDelegationUnlockPenalty(
